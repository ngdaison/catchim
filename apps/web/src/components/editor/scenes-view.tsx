...
export function ScenesView({ children }: { children: React.ReactNode }) {
	...
	const handleDeleteSelected = async () => {
		for (const sceneId of selectedScenes) {
			...
			if (!canDelete) {
				toast.error(reason || t("project.failedToDeleteScene"));
				continue;
			}
		...
		<Sheet>
			...
			<SheetContent>
				<SheetHeader>
					<SheetTitle>
						{isSelectMode
							? t("project.selectScenes", { count: selectedScenes.size })
							: t("project.scenes")}
					</SheetTitle>
					<SheetDescription>
						{isSelectMode
							? t("project.selectScenesToDelete")
							: t("project.switchScenesDescription")}
					</SheetDescription>
				</SheetHeader>
				<div className="flex flex-col gap-4 py-4">
					<div className="flex items-center gap-2">
						...
						>
							<ListCheck />
							{isSelectMode ? t("common.cancel") : t("project.select")}
						</Button>
						{isSelectMode && (
							<DeleteDialog
								...
								trigger={
									...
									>
										<Trash2 />
										{t("project.deleteProject")} ({selectedScenes.size})
									</Button>
								}
							/>
						)}
...
function DeleteDialog({
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{t("project.deleteScenes")}</DialogTitle>
					<DialogDescription>
						{t("project.deleteScenesDescription", { count })}
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
